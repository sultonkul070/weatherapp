...
function WaterApi(latitude, longitude) {
  ...
  fetch(Url)
    ...
    .then((datos) => {
      let Fecha = new Date();
      let FechaActualizado = new Date(
        datos.sys.sunset * 1000
      ).toLocaleDateString(`${datos.sys.country}`);
      ...
      document.querySelector(
        "#DireccionViento"
      ).innerText = `${datos.wind.deg}°`;

      document.querySelector(
        "section.MenuMobil #Velviento"
      ).innerText = `${datos.wind.speed}m/s`;
      document.querySelector(
        "section.MenuMobil #Humedad"
      ).innerText = `${datos.main.humidity}%`;
      document.querySelector(
        "section.MenuMobil #Nubosidad"
      ).innerText = `${datos.clouds.all}%`;
      document.querySelector(
        "section.MenuMobil #Visibilidad"
      ).innerText = `${datos.visibility}m`;
      document.querySelector(
        "section.MenuMobil #DireccionViento"
      ).innerText = `${datos.wind.deg}°`;
...
/* DIA MENOS UNO */
const LILessone = document.querySelector("#DiaMenos1");

function DiaMenosUno(latitude, longitude) {
  const Key = "08789ab932af5d6de716da1eaa4cfca7";

  //Fecha +1
  let DateNextOne = new Date();
  let Ano = DateNextOne.getFullYear();
  let Mes = DateNextOne.getMonth();
  let Dia = DateNextOne.getDate() - 1;
  ...
  fetch(Url)
    ...
    .then((datos) => {
      let DegreesDay = datos.hourly[0];
      let NameDay = new Date(Ano, Mes, Dia);
      let Celsius = Math.ceil(DegreesDay.temp - Kelvin);
      LILessone.innerHTML = `
                <img src="img/animated/${DegreesDay.weather[0].icon}.svg">
                <p>${DaysWeek[NameDay.getDay()]}</p>
                <p>${Celsius}ºc</p>
            `;
    })
    .catch((error) => {
...
/* DIA MENOS DOS*/
const LILesstwo = document.querySelector("#DiaMenos2");
...
/* DIA MENOS TRES*/
const LILessthree = document.querySelector("#DiaMenos3");
...
const LILessfour = document.querySelector("#DiaMenos4");
